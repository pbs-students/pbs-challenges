...
source "$(dirname "$0")/utility_functions.sh"
...
do
    case $opt in
        s)
			...
			if is_integer_number "$OPTARG"
			then
				start_multiplier="$OPTARG"
			else
				echo "invalid argument '$value' - must be an integer number" > /dev/tty
				exit 2
			fi
            ;;
        e)
			# store the end multiplier
            if is_integer_number "$OPTARG"
			then
...
then
	...
	if ! is_integer_number "$base_number" 
	then
...
multiplier_length=$(find_max $start_multiplier_length $end_multiplier_length)
...
result_length=$(find_max $start_result_length $end_result_length)
