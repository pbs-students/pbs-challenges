...
We need to get 3 things from 2 pages on Bart's website.

1. The current .zip for the most recent show installment
  * Contains the challenge solution and supplement files for the last installment's homework
...
The index of show notes is here: [PBS Home page](https://pbs.bartificer.net)
...
BTW: The challenge solution script will be named something like this:
...
$ ../edhowland/lib/bart-automation/get-show-id.sh 150
pbs-150-of-x--bash-script-plumbing

$ # Need to create a folder of that name
$ mkdir $(../edhowland/lib/bart-automation/get-show-id.sh 150)
```


### Now unzip the folder from the current show. (E.g. pbs 151)

- cd to that unzipped folder name (of the .zip without the .zip extension)
This folder will live in the ./tmp/ folder
...
and there may be files in there named with the 151 handle. Ignore them.
...
Note that the line matches the exact folder name you previously created but with
2 #'s just before it
...
the instructions you just pasted.
...
(Note: Add additional -m 'messages' if there are other files to mention )
```
...
Make sure there are no file or directory stragglers about.
...
In the future, I will probably create a new branch before starting this
adventure, then if it all goes smoothly, merge the branch back into main before pushing it back up to GitHub.  
...
If you are a Docker aficionado and prefer to use Docker for this:
...
For those in the Docker universe, the handle: 'rwcitek' is my buddy who has
